...
with vasocv1 as
    ...
    , max(case when itemid in (30044,30119,30309) then 1 else 0 end) as vaso -- dopamine
    -- the 'stopped' column indicates if a vasopressor has been disconnected
    , max(case when itemid in (30044,30119,30309) and stopped in ('Stopped','D/C','d') then 1 else 0 end) as vaso_stopped
    , max(case when itemid in (30044,30119,30309) and rate is not null then 1 else 0 end) as vaso_null
    , max(case
            when itemid = 30044 and wd.weight is null then rate / 80.0 -- this is rare, only affects a total of ~400 rows
            when itemid = 30044 then rate / wd.weight -- measured in mcgmin
            when itemid in (30119,30309) then rate -- measured in mcgkgmin ** there are clear errors, perhaps actually mcgmin
  ...
  left join `weightdurations` wd on cv.icustay_id = wd.icustay_id and cv.charttime between wd.starttime and wd.endtime
...
, vasomv as(select icustay_id, linkorderid, max(rate) as vaso_rate, sum(amount) as vaso_amount, min(starttime) as starttime, max(endtime) as endtime
  from `physionet-data.mimiciii_clinical.inputevents_mv` where itemid = 221289 -- epinephrine
