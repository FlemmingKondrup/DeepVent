...
with vasocv1 as
    ...
    , max(case when itemid in (30043,30307) and stopped in ('Stopped','D/C','d') then 1 else 0 end) as vaso_stopped
  ...
  from `physionet-data.mimiciii_clinical.inputevents_cv` where itemid in (30043,30307)
...
, vasomv as(select icustay_id, linkorderid, max(rate) as vaso_rate, sum(amount) as vaso_amount, min(starttime) as starttime, max(endtime) as endtime
  from `physionet-data.mimiciii_clinical.inputevents_mv` where itemid = 221662 -- dopamine
