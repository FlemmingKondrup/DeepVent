...
with vasocv1 as
    ...
    , max(case when itemid = 30051       and stopped in ('Stopped','D/C','d') then 1 else 0 end) as vaso_stopped
  ...
  from `physionet-data.mimiciii_clinical.inputevents_cv` where itemid = 30051 -- vasopressin
...
, vasocv4 as(
    ...
    , case
        ...
        --when (CHARTTIME - (LAG(CHARTTIME, 1) OVER (partition by icustay_id, vaso order by charttime))) > (interval '4 hours') then 1 else null
      end as vaso_start
FROM vasocv3)
...
, vasomv as(select icustay_id, linkorderid, max(rate) as vaso_rate, sum(amount) as vaso_amount, min(starttime) as starttime, max(endtime) as endtime
  from `physionet-data.mimiciii_clinical.inputevents_mv` where itemid = 222315 -- vasopressin 
